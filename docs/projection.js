...
function question(){
  ...
  shell(`<main class="projection-question">${modeTop()}<div class="projection-meta"><div><span>${esc(q.subject)}</span><span>${esc(q.chapter)}</span><span>Leçon ${esc(q.lesson)}</span></div><div>Question ${state.currentIndex+1}/${state.totalQuestions}</div>${timer}</div><h1>${esc(q.prompt)}</h1><div class="projection-answers">${q.choices.map((a,i)=>`<div class="projection-answer ans${i} ${revealed?(i===state.correctAnswer?'correct':'dim'):''}"><b>${String.fromCharCode(65+i)}</b><span>${esc(a)}</span></div>`).join('')}</div>${revealed&&state.mode!=='bts'?`<div class="projection-correction"><div class="projection-correction-score"><strong>${pct}%</strong><span>de bonnes réponses</span></div><div><b>✅ Bonne réponse : ${esc(q.choices[state.correctAnswer]||'')}</b><p>${esc(state.explanation||'')}</p>${state.mode==='duel'&&state.duel?.bothAnswered?`<strong class="duel-round-projection">${state.duel.roundWinnerName?`${esc(state.duel.roundWinnerName)} remporte la manche`:'Manche nulle'}</strong>`:''}</div></div>`:`<div class="projection-live-footer"><span>${state.mode==='bts'?'Réponses enregistrées':'Réponses reçues'}</span><strong>${state.responseCount}/${state.playerCount}</strong><div class="progress"><div style="width:${state.playerCount?state.responseCount/state.playerCount*100:0}%"></div></div></div>`}</main>`,true);startTimer();
}
...
async function refresh(){try{state=await rpc('mco_projection_state',{p_code:code});const boardKey=(state.leaderboard||[]).map(r=>`${r.rank}:${r.score}`).join(',');const teamKey=(state.teamLeaderboard||[]).map(r=>`${r.team}:${r.score}`).join(',');const k=`${state.status}-${state.currentIndex}-${state.revealed}-${state.responseCount}-${state.playerCount}-${boardKey}-${teamKey}-${state.duel?.roundWinnerName||''}`;if(k!==last){last=k;render()}}catch(e){root.innerHTML=`<div class="waiting"><div><h1>Projection indisponible</h1><p>${esc(e.message)}</p></div></div>`}}
