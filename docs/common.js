...
export function appShell(active='dashboard',profile=null){
  ...
  return `<aside class="sidebar premium-sidebar" data-sidebar>
    <div class="sidebar-head"><a class="brand-mini" href="./teacher.html"><img src="./assets/icons/icon-96.png" alt=""><span class="brand-mini-copy"><b>MCO Quiz Arena</b><small>NCR SOLUTIONS • BTS MCO</small></span></a><button id="sidebarToggle" class="sidebar-toggle" type="button" aria-label="Réduire le menu">${uiIcon('menu')}</button></div>
    <div class="side-section-label">ENSEIGNEMENT</div>
    <nav class="side-nav">${teacherNav.map(([key,href,ico,label,desc])=>`<a class="${active===key?'active':''}" href="${href}" data-nav-key="${key}">${uiIcon(ico)}<span class="nav-copy"><b>${label}</b><small>${desc}</small></span><span class="nav-arrow">${ICONS.chevron}</span></a>`).join('')}</nav>
    <div class="sidebar-quick"><a href="./teacher.html#create" class="sidebar-live-btn">${uiIcon('plus')}<span><b>Nouveau live</b><small>Créer une session</small></span></a><button class="sidebar-search-btn" id="openCommandPalette" type="button">${uiIcon('search')}<span>Navigation rapide</span><kbd>⌘K</kbd></button></div>
    <div class="teacher-card"><div class="teacher-avatar">${esc(initial)}</div><div class="teacher-card-copy"><b>${esc(name)}</b><small>${esc((profile?.role||'teacher').toUpperCase())}</small></div><button id="logoutBtn" class="logout-mini" title="Se déconnecter" aria-label="Se déconnecter">${uiIcon('logout')}</button></div>
    <div class="side-foot"><img src="./assets/icons/icon-64.png" alt=""><span>NCR SOLUTIONS<br><b>BTS MCO</b></span></div>
  </aside>`;
}
...
export function studentTopbar(student,active='home'){
  ...
  return `<header class="student-topbar"><a class="student-brand" href="./student.html"><img src="./assets/icons/icon-64.png" alt=""><span><b>MCO Quiz Arena</b><small>NCR Solutions • BTS MCO</small></span></a><nav class="student-main-nav">${links}</nav><div class="student-profile-zone"><a class="student-mini student-profile-chip" href="./student-profile.html" aria-label="Personnaliser mon avatar"><span class="student-mini-avatar">${avatarMarkup(student?.avatar||{},'avatar-svg avatar-topbar-svg',`Avatar de ${student?.displayName||'élève'}`)}</span><span class="student-mini-copy"><span>${esc(student?.displayName||'Élève')}</span><small>${esc(student?.className||'BTS MCO')}</small></span></a><button id="studentLogout" class="student-logout" type="button" aria-label="Se déconnecter">${uiIcon('logout')}<span>Déconnexion</span></button></div></header><nav class="student-mobile-dock" aria-label="Navigation élève">${links}</nav>`;
}
...
function teacherUtility(active){
  ...
  return `<header class="teacher-utility"><div class="teacher-utility-left"><button class="utility-menu" id="mobileMenuToggle" type="button" aria-label="Ouvrir le menu">${uiIcon('menu')}</button><div class="utility-page-icon">${uiIcon(meta.ico)}</div><div class="utility-copy"><small>ESPACE PROFESSEUR</small><b>${esc(meta.label)}</b></div></div><div class="teacher-utility-actions"><button class="utility-search" id="openCommandPaletteTop" type="button">${uiIcon('search')}<span>Rechercher une page ou une action</span><kbd>⌘K</kbd></button><a class="utility-live" href="./teacher.html#create">${uiIcon('plus')}<span>Nouveau live</span></a></div></header>`;
}
...
function openCommandPalette(){
  ...
  const items=teacherNav.map(([key,href,ico,label,desc])=>`<a href="${href}" class="command-item" data-command-item>${uiIcon(ico)}<span><b>${label}</b><small>${desc}</small></span><kbd>↵</kbd></a>`).join('');
  document.body.insertAdjacentHTML('beforeend',`<div class="command-backdrop" id="commandPalette"><div class="command-palette" role="dialog" aria-modal="true" aria-label="Navigation rapide"><div class="command-search">${uiIcon('search')}<input id="commandSearch" autocomplete="off" placeholder="Rechercher une page, un live, une action…"><kbd>ESC</kbd></div><div class="command-section-label">NAVIGATION</div><div class="command-list">${items}</div><div class="command-section-label">ACTION RAPIDE</div><div class="command-list"><a href="./teacher.html#create" class="command-item command-live" data-command-item>${uiIcon('live')}<span><b>Nouveau live</b><small>Créer une session maintenant</small></span><kbd>↵</kbd></a></div><div class="command-foot"><span><kbd>↑</kbd><kbd>↓</kbd> naviguer</span><span><kbd>↵</kbd> ouvrir</span></div></div></div>`);
  const backdrop=$('#commandPalette');
  backdrop.addEventListener('click',e=>{if(e.target===backdrop)closeCommandPalette()});
  const input=$('#commandSearch');input?.focus();
  let index=0;
  const visible=()=>$$('[data-command-item]',backdrop).filter(a=>!a.hidden);
  const mark=()=>{const list=visible();list.forEach((a,i)=>a.classList.toggle('selected',i===index));list[index]?.scrollIntoView({block:'nearest'})};
  input?.addEventListener('input',()=>{const q=input.value.trim().toLowerCase();$$('[data-command-item]',backdrop).forEach(a=>a.hidden=!!q&&!a.textContent.toLowerCase().includes(q));index=0;mark()});
  input?.addEventListener('keydown',e=>{const list=visible();if(!list.length)return;if(e.key==='ArrowDown'){e.preventDefault();index=(index+1)%list.length;mark()}else if(e.key==='ArrowUp'){e.preventDefault();index=(index-1+list.length)%list.length;mark()}else if(e.key==='Enter'){e.preventDefault();list[index]?.click()}});
  mark();
}
